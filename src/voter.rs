...
use crate::tui::{FrUiStatus, SharedState};
...
/// Wait for the user to confirm via the TUI modal.
async fn human_pause_tui(prompt: &str, state: &SharedState) {
    {
        let mut s = state.lock().unwrap();
        s.modal = Some(prompt.to_string());
...
/// TUI-aware version of `vote_one` that updates shared UI state as it progresses.
pub async fn vote_one_tui(
    page: &Page,
    fr_id: &str,
    fr_index: usize,
    config: &Config,
    ...
    {
        let mut s = state.lock().unwrap();
        s.frs[fr_index].status = FrUiStatus::InProgress;
    ...
    if let Err(e) = page.goto(&url).await {
        ...
        let mut s = state.lock().unwrap();
        s.frs[fr_index].status = FrUiStatus::Failed;
        s.frs[fr_index].message = result.message.clone();
        s.log(format!("[vote] {} — failed to navigate: {}", fr_id, e));
        return result;
    }
    ...
    let html = match page.content().await {
        ...
        Err(e) => {
            ...
            s.frs[fr_index].status = FrUiStatus::Failed;
            s.frs[fr_index].message = result.message.clone();
            s.log(format!("[vote] {} — failed to read page: {}", fr_id, e));
            return result;
        }
    };
    ...
    if already_voted {
        {
            let mut s = state.lock().unwrap();
            s.log(format!("[vote] {} — already voted", fr_id));
        }
        ...
        if config.company.is_some() {
            ...
            let mut s = state.lock().unwrap();
            s.frs[fr_index].status = FrUiStatus::AlreadyVoted;
            s.frs[fr_index].message = result.message.clone();
            return result;
        }
    ...
    if !vote_button_found {
        let result = VoteResult {
            ...
            url,
            status: VoteStatus::Failed("vote button not found".into()),
            message: "Could not locate a Vote button on the page. Selectors may need updating."
                .into(),
            timestamp,
        };
        let mut s = state.lock().unwrap();
        s.frs[fr_index].status = FrUiStatus::Failed;
        s.frs[fr_index].message = result.message.clone();
        s.log(format!("[vote] {} — vote button not found", fr_id));
        return result;
    }
    ...
    if config.dry_run {
        {
            let mut s = state.lock().unwrap();
            s.log(format!(
                "[vote] {} — DRY RUN: vote button located, not clicking",
                fr_id
            ));
        }
        let result = VoteResult {
        ...
        let mut s = state.lock().unwrap();
        s.frs[fr_index].status = FrUiStatus::DryRunOk;
        s.frs[fr_index].message = result.message.clone();
        return result;
    }

    // Live mode: confirm with human via TUI modal before clicking
    {
        let mut s = state.lock().unwrap();
        s.log(format!(
            "[vote] {} — waiting for confirmation to click vote",
            fr_id
        ));
    }
    human_pause_tui(
    ...
    match click_vote_button(page).await {
        Ok(_) => {
            {
                let mut s = state.lock().unwrap();
                s.log(format!("[vote] {} — vote clicked", fr_id));
            }
            ...
            let mut s = state.lock().unwrap();
            s.frs[fr_index].status = FrUiStatus::Voted;
            s.frs[fr_index].message = result.message.clone();
            result
        }
        Err(e) => {
            ...
            s.frs[fr_index].status = FrUiStatus::Failed;
            s.frs[fr_index].message = result.message.clone();
            s.log(format!("[vote] {} — failed to click vote: {}", fr_id, e));
            result
        }
    }
}
